...
// Update settings tab content with backup status and FAQ (v3.9.0)
...
function scrollToFAQ() {
    ...
    if (faqSection) {
        ...
        // Auto-expand first section (Data Backup & Recovery)
        setTimeout(() => {
...
function exportToCSV() {
    ...
    updateBackupTimestamp(); // Record backup (v3.9.0)
...
window.addEventListener('load', async function () {
    try {
        ...
        loadSummaryState();
        updateCurrencyDisplay();
        loadBackupTimestamp(); // Load backup status (v3.9.0)
        checkInstallPrompt();
    } catch (err) {
...
if ('serviceWorker' in navigator && window.location.protocol !== 'file:') {
    ...
    navigator.serviceWorker.register('sw.js')
        .then(registration => {
            ...
            registration.update()
                .catch(err => console.warn('⚠️ Service Worker update check failed:', err));

            // Check for updates when user returns from another tab
