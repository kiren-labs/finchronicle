...
class ReactiveState {
  ...
  undo() {
    ...
    if (this._listeners.has(change.key)) {
      this._listeners
        .get(change.key)
        .forEach((cb) => cb(change.oldValue, change.newValue));
    }
    return true;
  }
  ...
  redo() {
    ...
    if (this._listeners.has(change.key)) {
      this._listeners
        .get(change.key)
        .forEach((cb) => cb(change.newValue, change.oldValue));
    }
    return true;
  }
}
...
describe("TransactionService", () => {
  ...
  test("calculates month summary correctly", () => {
    const summary = TransactionService.getMonthSummary(
      mockTransactions,
      "2026-03",
    );
