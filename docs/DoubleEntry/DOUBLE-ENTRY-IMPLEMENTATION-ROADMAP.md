  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (e) => {
      ...
      if (!db.objectStoreNames.contains("journal_entries")) {
        const entryStore = db.createObjectStore("journal_entries", {
          keyPath: "id",
        });
...
async function saveJournalEntry(entry) {
  ...
  return new Promise((resolve, reject) => {
    const request = entry.id
      ? tx.objectStore("journal_entries").put(toSave)
...
async function migrateV3ToV4(options = {}) {
  ...
  try {
    ...
    if (!balance.isBalanced) {
      throw new Error(
        `Trial balance failed: DR=${balance.totalDebits}, CR=${balance.totalCredits}`,
      );
    }
...
await testMigration([{ type: "expense", category: "UNKNOWN", amount: 25 }], {
  expected: 3,
  fallback: "5900",
});

// Test 5: Very old data (2015)
await testMigration([{ date: "2015-01-01", type: "expense", amount: 100 }], {
  expected: 3,
  dateHandled: true,
});

// Test 6: Large dataset (10000 transactions)
await testMigration(largeDataset, { expected: 10001, performOk: true });

// Test 7: Negative amounts (should fail validation)
await testMigration([{ amount: -50 }], { expected: "error" });
```
...
function getTransactionType(entry) {
  const accountTypes = entry.entries.map(
    (e) => accounts.find((a) => a.id === e.accountId)?.type,
  );
...
<div id="migrationModal" class="modal">
  <div class="modal-content">
    ...
    <div class="modal-actions">
      <button class="btn btn-secondary" onclick="closeMigrationModal()">
        Not Now
      </button>
