...
export async function checkRecurringTransactions() {
    ...
    const due = state.recurringTemplates.filter(t => t.enabled && t.nextDueDate <= todayStr);
    ...
    for (let i = 0; i < due.length; i++) {
        ...
        if (!validation.valid) {
            console.warn('[Recurring] Skipped invalid template:', template.id, validation.errors);
            continue;
        }
        ...
        template.nextDueDate = computeNextDueDate(template.frequency, template.dayOfMonth, template.nextDueDate);
    ...
    if (generated > 0) {
        ...
        showMessage(`${generated} recurring transaction${generated > 1 ? 's' : ''} added`);
    }
...
export function renderRecurringSection() {
    ...
    document.getElementById('addRecurringBtn')
...
export function openRecurringModal(id = null) {
    ...
    if (id !== null) {
        ...
        if (template) {
            ...
            document.getElementById('recurringStartDate').value = template.nextDueDate;
...
export async function saveRecurringTemplate() {
    ...
    const dayOfMonth = (frequency === 'monthly' && dayRaw) ? parseInt(dayRaw, 10) : null;
    if (dayOfMonth !== null && (isNaN(dayOfMonth) || dayOfMonth < 1 || dayOfMonth > 28)) {
    ...
    if (isEditing) {
        const template = state.recurringTemplates.find(t => t.id === _editingRecurringId);
        if (!template) return;
        template.type = type;
...
async function deleteRecurring(id) {
    ...
    if (!confirm(`Delete "${label}"?\n\nAlready-created transactions will not be affected.`)) return;
