...
export function updateSummary() {
    ...
    const { income, expense, count } = state.transactions.reduce((acc, t) => {
...
export function calculateMoMDelta(currentMonth, previousMonth) {
    ...
    if (previousMonth === 0) {
        ...
        return { abs: currentMonth, pct: null, direction: currentMonth > 0 ? 'up' : 'down' };
    }
...
export function calculateBudgetHealth(month) {
    ...
    if (variancePercent < 20) {
        status = 'On Track'; statusIcon = 'check-line';
    } else if (variancePercent < 50) {
        status = 'Caution'; statusIcon = 'alert-line';
    } else {
        status = 'Over Pace'; statusIcon = 'close-circle-line';
    }

    return { dailyPace, daysRemaining, projectedMonthEnd, status, statusIcon, variancePercent };
}

export function renderMonthlyInsights() {
    const targetMonth = state.insightsMonth === 'current' ? new Date().toISOString().slice(0, 7) : state.insightsMonth;
